import { Button, HStack, Show, Spacer, Text, VStack, useColorMode } from "@chakra-ui/react";
...
const JournalListItem = ({ journal }: Props) => {
  ...
  return (
      <VStack
        ...
        gap={5}>
        <Button variant={'unstyled'} height={'auto'} textAlign={'left'} onClick={() => navigate('/everything/journals/' + journal?._id)} width={'100%'} gap={5}>
          <VStack gap={2} paddingY={2} align={'left'}>
            <HStack width={'100%'} gap={5}>
              <Text fontWeight={'bold'} overflow={'hidden'}>{ journal?.title }</Text>
              <Spacer />
              <Text fontWeight={'light'} width={'6rem'}>{ formattedDate }</Text>
            </HStack>
            <Show above={'lg'}>
