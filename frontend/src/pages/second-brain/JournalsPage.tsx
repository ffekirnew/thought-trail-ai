import { Button, Heading, IconButton, Menu, Text, MenuButton, MenuItem, MenuList, Spacer, VStack, Show, Flex } from '@chakra-ui/react'
...
const JournalsPage = () => {
  ...
  return <VStack height={'100%'} align={'left'}>
    ...
    <Flex flexWrap={'wrap'} paddingBottom={3} gap={3}>
      ...
      <Show above={'lg'}>
        ...
        <IconButton aria-label={'change to grid view'} as={Button} variant={'outline'}><BiListOl /></IconButton>
      </Show>
    </Flex>
    { isLoading &&
      journalSkeletons.map((skeleton) => <JournalListItemSkeleton key={skeleton} />)
    }
    { journals &&
      journals?.map((journal) => <JournalListItem key={journal._id} journal={journal} />)
    }
    {
      !isLoading && journals?.length == 0 &&
      <VStack height={'100%'} justifyContent={'center'}>
        <Button variant={'ghost'} onClick={onAddNewJournal}>
          <Text color={'gray.600'}>You don't have a journal. Start your journaling journey today!</Text>
        </Button>
      </VStack>
    }
  </VStack>
